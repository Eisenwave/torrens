...
public class ArrayBlockStructure extends AbstractArray3 implements BlockStructure, Serializable, Cloneable {
    ...
    public ArrayBlockStructure(ArrayBlockStructure copyOf) {
        this(copyOf.sizeX, copyOf.sizeY, copyOf.sizeZ, copyOf.flags);
        forEachPos((pos) -> setBlock(pos, copyOf.getBlock(pos)));
        if (hasBiomes())
            forEachPos((x, y, z) -> setBiome(x, z, copyOf.getBiome(x, z)));
        if (hasLight())
            forEachPos(pos -> setBlockLight(pos, copyOf.getLight(pos)));
    }
    ...
    public byte getBiome(Vertex2i pos) {
        return getBiome(pos.getX(), pos.getY());
    }
    ...
    public byte getLight(Vertex3i pos) {
    ...
    public boolean equals(ArrayBlockStructure array) {
        return this.getSizeX() == array.getSizeX()
    ...
    public ArrayBlockStructure clone() {
        return new ArrayBlockStructure(this);
    }
