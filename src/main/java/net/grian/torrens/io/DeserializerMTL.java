...
import net.grian.torrens.convert.ConverterImageToTexture;
...
public class DeserializerMTL implements TextDeserializer<Void> {
    ...
    private final OBJMaterialLibrary mtllib;
    private final File mapDir;
    
    private OBJMaterial newmtl;
    
    public DeserializerMTL(@Nonnull OBJMaterialLibrary mtllib, @Nonnull File mapDir, @Nullable Logger logger) {
        this.mtllib = Objects.requireNonNull(mtllib);
        this.mapDir = Objects.requireNonNull(mapDir);
        this.logger = logger;
    }
    
    public DeserializerMTL(OBJMaterialLibrary mtllib, File mapDir) {
    ...
    private void readLine(int number, String line) throws FileSyntaxException {
        ...
        if (args[0].equals("newmtl")) {
            this.newmtl = new OBJMaterial(mtllib, args[1]);
