...
public class SerializerMTL implements TextSerializer<MTLLibrary> {

    @Override
    public void toWriter(MTLLibrary mtllib, Writer writer) throws IOException {
        try (BufferedWriter buffWriter = new BufferedWriter(writer)) {
            toWriter(mtllib, buffWriter);
        }
    }

    public void toWriter(MTLLibrary mtllib, BufferedWriter writer) throws IOException {
        for (MTLMaterial material : mtllib)
            writeMaterial(material, writer);
    }

    private void writeMaterial(MTLMaterial material, BufferedWriter writer) throws IOException {
