...
public class SerializerICO implements Serializer<BufferedImage[]> {
    ...
    public void toStream(BufferedImage[] icons, LittleDataOutputStream stream) throws IOException {
        if (icons.length == 0)
        ...
        IconDirEntry[] entries = new IconDirEntry[icons.length];
    ...
    @NotNull
    private static IconDirEntry makeBMPEntry(byte[] bmp, int width, int height, int offset) {
        ...
        return new IconDirEntry(width, height, palette, planes, bpPixel, bmp.length, offset);
    }
    
    @NotNull
    private static IconDirEntry makePNGEntry(byte[] png, int width, int height, int offset) {
        /*
        ...
        */
        ...
        return new IconDirEntry(width, height, 0, 0, 0, png.length, offset);
    }
    
    private void writeEntry(IconDirEntry entry, LittleDataOutputStream stream) throws IOException {
